...
train_dataset = dataloader_SRUNET.microCT_Dataset(train_files,train_or_test = 'train',
...
# If need to generate pathes for the firs time then put true for the need_patches
test_dataset = dataloader_SRUNET.microCT_Dataset(test_files,train_or_test = 'test',
                                         HR_patch_size=patch_size,
                                         transform = None,
                                          need_patches=False)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            for data, target in test_data_loader:
                ...
                metric1 = skimage.metrics.peak_signal_noise_ratio(target, output)
                PNSR.append(metric1)
                t_counter +=1
                if counter>stop:
...
#save model
def save_checkpoint(state, filename="my_SRUNET_checkpoint.pth.tar"):
